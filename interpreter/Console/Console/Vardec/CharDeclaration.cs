 ...
 *      12-04-2015 14:26 to 12-06-2015 9:04 B2s modified SIMBOLO
 *      12-09-2015 1:45 to 2:15: Julius modified regex, Check, and Analyze
 *      12-11-2015 4:20 to 5:27: Julius modified regex so that it can take variables for initialization
...
namespace Interpret.Vardec{
	class CharDeclaration : Submodule{
        public static Regex CHAR_VALUE = new Regex(".", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        public const string DATA_TYPE = "SIMBOLO";
		private Regex CharRegex;
		private int DefaultValue;
        private Match Matcher;

		public CharDeclaration(){
            CharRegex = new Regex("^\\s*(" + DATA_TYPE + ")\\s+(" + Constant.IDENTIFIER.ToString() + ")\\s*(\\s+(" + Constant.INIT.ToString() + ")\\s+(\'(" + CHAR_VALUE.ToString() + ")\'|" + Constant.IDENTIFIER.ToString() + "))?\\s*(" + CommentModule.ONE_LINE_REGEX +")?$", RegexOptions.Compiled | RegexOptions.IgnoreCase);
            DefaultValue = 0;
		}

		public bool Check(string line){
            Matcher = CharRegex.Match (line);
		...
		public void Run(List<Lexeme> lex){
            if (Program.Symbol.ContainsKey(lex[1].Name))
                ...
                Program.Symbol.Add(lex[1].Name, new ValueClass(((char)DefaultValue).ToString(), lex[0].Name));
